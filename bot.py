...
# Обработчик для аннулирования работ - должен быть ПЕРВЫМ, чтобы проверяться раньше других
...
async def run_bot():
    ...
    logging.info("✅ Callback handler для аннулирования работ зарегистрирован через декоратор (первым)")
    
    # Явно регистрируем обработчик cancel_work ПЕРВЫМ (до других обработчиков)
    # Это важно для aiogram 2.x, где порядок обработчиков имеет значение
    # В aiogram 2.x обработчики проверяются в порядке регистрации
    dp.register_callback_query_handler(
        cancel_work_callback_handler,
        lambda c: c.data and (c.data.startswith('cancel_work:') or c.data.startswith('cancel:'))
    )
    logging.info("✅ Обработчик аннулирования явно зарегистрирован ПЕРВЫМ через register_callback_query_handler")
