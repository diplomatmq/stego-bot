...
# pool_size: базовое количество соединений в пуле (уменьшено для экономии соединений)
...
# Блокировка для предотвращения одновременных вызовов init_db
...
async def init_db():
    ...
    async with _init_db_lock:
        ...
        # Retry логика с экспоненциальной задержкой для TooManyConnectionsError
        max_retries = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Если успешно, помечаем как инициализированную
                ...
                if "TooManyConnectionsError" in str(type(e).__name__) or "TooManyConnections" in str(e):
                    if attempt < max_retries - 1:
                        wait_time = retry_delay * (2 ** attempt)  # Экспоненциальная задержка
                        print(f"⚠️ Слишком много соединений. Повторная попытка {attempt + 1}/{max_retries} через {wait_time} сек...")
